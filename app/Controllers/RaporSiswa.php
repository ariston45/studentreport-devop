...
class RaporSiswa extends BaseController
{
		...
		switch ($this->session->get('u_rules_access')) {
			case 'MGNT_SUPERADMIN':
				$this->partial = [
					...
					'content'	=> [
						'content_menu' => '',
						'content_body' => 'view_features/rapor/pg_rapor',
			...
			case 'MGNT_SUPERADMIN':
				$this->partial = [
					...
					'content'	=> [
						...
						'content_menu' => 'view_features/rapor/pg_menu',
						'content_body' => 'view_features/rapor/pg_cari_siswa'
					],
					'data' => [
						'sekolah' => $sekolah,
					]
			...
			case 'MGNT_SUPERADMIN':
				$this->partial = [
					...
					'content'	=> [
						...
						'content_menu' => 'view_features/rapor/pg_menu',
						'content_body' => 'view_features/rapor/pg_list_rapor_siswa'
					],
					'data' => [
						'school' => $school,
						'thajaran' => $th_ajaran,
	...
	public function DetailRaporSiswa($stri,$stra,$stre)
	{
		...
		$sum_nilai = 0 ;
		$i = 1 ;
		$param['evaluasi'] = $evaluasi[0]['cat_id'];
		$param['tahun'] = $evaluasi[0]['cat_acad_id'];
		$param['kelas'] = $nilai[0]['fds_class'];
		$param['id_siswa'] = $stra;
		// 
		foreach ($nilai as $key => $value) {
			$sum_nilai = $sum_nilai + $value['fds_score'];
			$count_nilai = $i;
		...
		//
		$peringkat = $this->PeringkatKelas($param);
		//  
		switch ($this->session->get('u_rules_access')) {
			case 'MGNT_SUPERADMIN':
				$this->partial = [
					...
					'content'	=> [
						...
						'pg_link' => base_url('rapor-siswa/' . $stri),
						'content_menu' => 'view_features/rapor/pg_menu',
						'content_body' => 'view_features/rapor/pg_rapor_siswa'
					],
					'data' => [
						'sekolah' => $sekolah,
						'siswa' => $siswa[0],
						...
						'group' => $group,
						'jumlah' => $sum_nilai,
						'rata-rata' => $sum_nilai/$count_nilai,
						'peringkat' => $peringkat
					]
				];
				return view('layout/main_layout', $this->partial);
	...
	#####
	public function PeringkatKelas($data)
	{
		// echo 'id siswa : '.$data['id_siswa'].'<br>';
		$nilaiSiswa = $this->RaporModel->NilaiEvaluasiKelasSiswa($data);
		// 
		foreach ($nilaiSiswa as $key => $value) {
			$nilaiGroup[$value['fds_std_number']][] = $value;
		}
		//
		$i_siswa = 1; 
		foreach ($nilaiGroup as $k_siswa => $value) {
			$score = 0 ;
			$count_siswa = $i_siswa;
			foreach ($value as $k_nilai => $subvalue) {
				$score = $score + $subvalue['fds_score'];
				$siswa[$i_siswa]['id_siswa'] = $k_siswa;
				$siswa[$i_siswa]['Jumlah_nilai'] = $score;
			};
			$i_siswa++;
		}
		...
		foreach ($siswa as $key => $value) {
			// echo $peringkat.' .'.$value['id_siswa'].' nilai : '.$value['Jumlah_nilai'];
			if ($value['id_siswa'] == $data['id_siswa']) {
				$text = ' Peringkat '.$peringkat.' dari '.$count_siswa.' siswa';
			}
			// echo '<br>';
			// if ($value['id_siswa'] == $data['id_siswa']) {
			// 	$text = 'Peringkat '.$peringkat.' dari '.$count_siswa.' siswa';
